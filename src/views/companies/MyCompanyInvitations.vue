		...
		<div class="max-w-7xl mx-auto">
			...
			<div class="mb-6 sm:mb-8">
				...
				<p class="text-sm sm:text-base text-gray-600">
					Manage pending invitations for your company
				</p>
			</div>
			...
			>
				...
				<p class="text-gray-600 font-medium">Loading invitations...</p>
			</div>
			...
			>
				...
				<p class="text-gray-600 max-w-md mx-auto">
					You don't have a company associated with your account. Create
					or join a company to manage invitations.
				</p>
			</div>
...
import InvitationsList from '@/components/companies/InvitationsList.vue';
import { getMyCompanyInvitations } from '@/api/companies.api';
...
async function loadInvitations() {
	...
	try {
		const result = await getMyCompanyInvitations();
		if (result.success !== false) {
			invitations.value = result.data || result;
		} else {
			loadError.value = result.message || 'Failed to load invitations. Please try again.';
		}
	} catch (err) {
		loadError.value = err?.message || 'An unexpected error occurred. Please try again.';
	} finally {
		loading.value = false;
	}
}
