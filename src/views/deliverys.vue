		...
		<div class="max-w-5xl mx-auto space-y-6">
			...
			<div class="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
				...
				>
					...
					<div class="flex items-center gap-2">
						<label for="sort-select" class="text-gray-600"
							>Sort by:</label
						>
						<select
							id="sort-select"
			...
			>
				...
				<p class="text-gray-600">
					Your deliveries will appear here when they arrive
				</p>
			</div>
			...
			<div v-else class="space-y-4 relative">
				...
				>
					<div class="text-center">
						<svg
							class="w-12 h-12 mx-auto text-[#0D65AE] animate-spin mb-2"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
				...
				>
					...
					>
						...
						>
							<img
								v-if="getCompanyLogo(delivery.company.name)"
								:src="getCompanyLogo(delivery.company.name)"
								:alt="delivery.company.name"
						...
						<svg
							...
							:class="{ 'rotate-180': openedTabs.includes(idx) }"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					<div
						v-show="openedTabs.includes(idx)"
						...
						<button
							...
							:class="[
								...
								page === pagination.currentPage
									? 'bg-[#0D65AE] text-white border-[#0D65AE]'
...
function getCompanyLogo(companyName) {
	const logos = {
		...
		"Australia Post":
			"https://upload.wikimedia.org/wikipedia/commons/thumb/3/34/Australia_Post_logo.svg/200px-Australia_Post_logo.svg.png",
		...
		if (idx !== -1) {
			...
			if (mapContainer && delivery.info?.location) {
				if (mapContainer._leaflet_map) {
					...
					mapContainer._leaflet_map.setView(
						[latLng.lat, latLng.lng],
						5,
					);
