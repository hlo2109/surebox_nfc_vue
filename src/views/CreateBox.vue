            ...
            <form @submit="handleSubmit" class="space-y-4">
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input v-model="codebox_id" type="text" class="w-full border rounded px-3 py-2"
                            required />
                        <button type="button" @click="scanQR"
                            class="p-2 bg-gray-200 rounded hover:bg-gray-300" title="Read QR">
                        ...
                        <button type="button" @click="readNFC"
                            class="p-2 bg-gray-200 rounded hover:bg-gray-300" title="Read NFC">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                stroke-width="1.5" stroke="currentColor"
                ...
                <div>
                    ...
                    <input v-model="phone" type="text" class="w-full border rounded px-3 py-2" required placeholder="E.g.: +61 4XX XXX XXX or 04XX XXX XXX" :class="{'border-green-500': phoneValid, 'border-red-500': !phoneValid && phone}" />
                    <div v-if="phone && !phoneValid" class="text-xs text-red-500 mt-1">Invalid Australian phone format</div>
                </div>
                <div>
                    <label class="block mb-1 font-medium">Address</label>
                    <input v-model="address" type="text" class="w-full border rounded px-3 py-2" required autocomplete="off" />
                    <ul v-if="showSuggestions && addressSuggestions.length" class="bg-white border rounded shadow mt-1">
                        <li v-for="suggestion in addressSuggestions" :key="suggestion.formattedAddress" @click="selectAddress(suggestion)" class="px-3 py-2 cursor-pointer hover:bg-blue-100">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" @click="getCurrentLocation" class="p-2 bg-gray-200 rounded hover:bg-gray-300" title="Use my location">
                ...
                <div>
                    ...
                    <button type="button" @click="startCamera" class="bg-green-500 text-white px-3 py-2 rounded mb-2">Activate camera</button>
                    <div v-if="showCamera" class="mb-2">
                        <video ref="videoElement" style="width:100%;max-height:300px;border-radius:8px;" autoplay></video>
                        <div class="flex gap-2 mt-2">
                            <button type="button" @click="takePhoto" class="bg-blue-500 text-white px-3 py-2 rounded">Take photo</button>
                            <button type="button" @click="stopCamera" class="bg-gray-500 text-white px-3 py-2 rounded">Close camera</button>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-2 mt-2">
                        <img v-for="(img, i) in images" :key="i" :src="img.url" class="w-24 h-24 object-cover rounded border" />
                    </div>
                </div>
                <div>
