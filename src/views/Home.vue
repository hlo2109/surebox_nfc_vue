<template>
	<div class="bg-gray-50 p-4 sm:p-6 lg:p-8">
		<div class="max-w-7xl mx-auto">
					...
					<div class="flex items-center justify-between">
						<div class="flex-1">
							...
							>
								...
								<span
									>{{ stats.pendingDeliveries }} pending</span
								>
							</p>
						</div>
						<div
			...
			<!-- Main Content Grid -->
			<div
				class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8"
			>
				<!-- Recent Deliveries -->
				<div
				...
				>
					...
					<div
						v-else-if="recentDeliveries.length === 0"
				...
				>
					...
					<div class="space-y-3">
						...
						<div
							class="bg-[#0D65AE]/5 p-4 rounded-lg border border-[#0D65AE]/20"
						>
							<div class="flex items-start gap-3">
								<div
									class="bg-[#0D65AE] p-2 rounded-lg flex-shrink-0 border border-[#0D65AE]"
								>
									<svg
								...
								<div class="flex-1">
									...
									<p class="text-xs text-gray-600 mb-2">
										Learn how to manage your boxes and track
										deliveries.
									</p>
									<button
			...
			<div class="bg-white rounded-xl border border-gray-200 p-5 sm:p-6">
				...
				>
					...
					<p class="text-gray-500 mb-4">
						You haven't registered any boxes yet
					</p>
					<button
						@click="$router.push('/create-box')"
				...
				>
					...
					>
						<div class="flex items-start justify-between mb-3">
							<div
								class="bg-[#0D65AE]/10 p-2.5 rounded-lg border border-[#0D65AE]/20"
							>
								<svg
...
const stats = computed(() => ({
	...
	activeDeliveries: deliveries.value.filter((d) => d.state !== "delivered")
		.length,
	pendingDeliveries: deliveries.value.filter((d) => d.state === "pending")
		.length,
	deliveredPackages: deliveries.value.filter((d) => d.state === "delivered")
		.length,
...
async function fetchDeliveries() {
	try {
		...
		notificationCount.value = deliveries.value.filter(
			(d) => d.state !== "delivered",
		).length;
	} catch (err) {
...
async function loadDashboardData() {
	...
	try {
		await Promise.all([fetchBoxes(), fetchDeliveries()]);
