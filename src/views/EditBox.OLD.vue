		...
		<div class="max-w-4xl mx-auto">
			...
			<div class="mb-6 sm:mb-8">
				<div class="flex items-center gap-3 mb-2">
					...
					<div>
						<h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Box</h1>
						<p class="text-sm text-gray-600">Update box information</p>
					</div>
				</div>
			</div>
				...
				<form @submit="handleSubmit" class="space-y-8">
					...
					<div class="space-y-4">
						...
						<div class="space-y-3">
							...
							<div class="flex gap-2">
								<input v-model="codebox_id" type="text"
					...
					<div class="space-y-4">
						...
						<div class="space-y-3">
							...
							<input v-model="phone" type="text" placeholder="E.g.: +61 4XX XXX XXX or 04XX XXX XXX"
								:class="['w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#0D65AE] focus:border-[#0D65AE] outline-none transition-all', phone && !phoneValid ? 'border-red-500' : 'border-gray-300']"
								required />
							<p v-if="phone && !phoneValid" class="text-sm text-red-600">Invalid Australian phone format
							</p>
							<p v-else class="text-sm text-gray-500">Enter a valid Australian phone number</p>
						</div>
					</div>
...
// Phone validation
watch(phone, (newVal) => {
	phoneValid.value = isValidPhoneNumber(newVal, 'AU')
})
...
function initializeMap() {
	nextTick(() => {
		if (mapContainer.value && lat.value && lng.value) {
			...
			L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19, attribution: '© OpenStreetMap contributors' }).addTo(map.value)
			L.marker([lat.value, lng.value]).addTo(map.value).bindPopup('📍 Selected location').openPopup()
		}
	})
}
...
async function startCamera() {
	...
	try {
		videoStream.value = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
		if (videoElement.value) videoElement.value.srcObject = videoStream.value
		toast.add({
			severity: 'info',
...
async function handleSubmit(e) {
	...
	error.value = ''
	const payload = { codebox_id: codebox_id.value, phone: phone.value, address: address.value, geolocation: geolocation.value, images: images.value.map(i => i.url), description: description.value }
	try {
		await axios.put(`${API_BASE_URL}/user-code-box/${boxId.value}`, payload)
		toast.add({
			severity: 'success',
			summary: 'Box Updated',
			detail: 'Your box information has been saved successfully',
			life: 2000
		})
		setTimeout(() => {
		...
		toast.add({
			severity: 'error',
			summary: 'Save Failed',
			detail: err.response?.data?.message || 'Error saving the box. Please try again.',
			life: 5000
		})
...
onMounted(async () => {
	try {
		...
		if (data.geolocation) {
			const [latVal, lngVal] = data.geolocation.split(',').map(Number)
			lat.value = latVal
			lng.value = lngVal
			showMap.value = true
			initializeMap()
		...
		toast.add({
			severity: 'success',
			summary: 'Box Loaded',
			detail: 'Box information loaded successfully',
			life: 2000
		})
	} catch (err) {
		toast.add({
			...
			detail: 'Error loading box data. Please try again.',
			life: 4000
		})
		error.value = 'Error loading box data'
	}
})
</script>
