<template>
  <!-- Navbar start -->
  <nav id="navbar" class="sticky top-0 z-40 flex w-full flex-row justify-end bg-primary-500 px-4 sm:justify-between items-center">
    ...
    <h3 class="sticky top-0 z-40 flex w-full flex-row  bg-primary-500 px-4 sm:justify-between text-white font-black text-2xl" ><router-link to="/">Surebox</router-link></h3>
    <button @click="toggleSidebar" type="button" class="py-4 text-2xl text-white hover:text-gray-200">
      <svg v-if="!sidebarOpen" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-8 w-8">
        <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
      </svg>
      <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-8 w-8">
  ...
  <div id="containerSidebar" class="z-40 flex min-h-screen overflow-hidden bg-gray-100">
        ...
        <div class="px-4 pb-6">
          ...
          <nav  id="sidebar"
            ...
            <ul class="mb-8 text-sm font-medium">
              ...
              <!-- Cerrar sesion -->
              <li>
                <button @click="logout" class="flex items-center rounded py-3 pl-3 pr-4 text-gray-50 hover:bg-primary-200 w-full  cursor-pointer">
    ...
    <div class="p-3 transition-all duration-300 relative z-2"
         :class="sidebarOpen ? 'lg:w-3/4 lg:left-80 w-full relative' : 'w-full lg:left-0 relative'"
         style="top:0;">
      <main>
        <router-view />
      </main>
